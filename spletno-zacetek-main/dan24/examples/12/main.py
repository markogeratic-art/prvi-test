...
# Dodajanje CORS middleware za omogočanje cross-origin zahtev
# CORS (Cross-Origin Resource Sharing) je mehanizem, ki omogoča ali prepoveduje spletni strani dostop do virov iz druge domene
# Brez CORS bi brskalnik blokiral zahteve iz ene domene na drugo iz varnostnih razlogov
# Primer: Če je frontend na http://localhost:3000, API pa na http://localhost:8000, CORS omogoči komunikacijo
# V tem primeru dovolimo vse izvorne domene, metode in glave za demonstracijo - v produkciji omejite na specifične domene!
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Dovoli vse izvorne domene (primer: ["http://localhost:3000", "https://moja-app.com"])
    allow_credentials=True,  # Dovoli pošiljanje piškotkov/avtentikacijskih podatkov
    allow_methods=["*"],  # Dovoli vse HTTP metode (GET, POST, PUT, DELETE itd.)
    allow_headers=["*"],  # Dovoli vse glave v zahtevah
)
